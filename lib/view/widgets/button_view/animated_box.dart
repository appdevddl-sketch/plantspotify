    ...
    transitionBuilder: (context, a1, a2, widget) {
      if(isFlip) {
        return Rotation3DTransition(
          ...
          animation: Tween<double>(begin: math.pi, end: 2.0 * math.pi).animate(CurvedAnimation(parent: a1, curve:const Interval(0.0, 1.0, curve: Curves.linear))),
...
class Rotation3DTransition extends AnimatedWidget {
  ...
  final Widget ?child;
  const Rotation3DTransition({Key? key, required Animation<double> animation, this.alignment = Alignment.center, this.child,}) : super(key: key, listenable: animation);
  @override
